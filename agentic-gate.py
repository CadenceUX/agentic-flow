...
Usage:
  ...
  agentic-gate.py status [session_id] [--preview [PATH]]
                                                # print active state + how it's armed;
                                                # --preview also writes an HTML status
                                                # page for the current state (a pure
                                                # read — no switch, nothing changes)
...
def _build_switch_preview_html(manifest, prev_state, new_env_name, mode="switch"):
    """Deterministic HTML fragment (no <html>/<head>/<body> — the Artifact
    tool supplies that) for a status page: what's currently loaded for the
    active environment (including the shared tier every environment gets
    silently), where each declared item actually lives on disk, and which
    hooks are the ones enforcing it. Generated by this script from the
    real manifest — not something the calling model should hand-author or
    paraphrase.

    mode="switch" (the default, used by `switch --preview`) additionally
    shows what changed: a from/to banner and a 'Switched out of' section.
    mode="status" (used by `status --preview`, a pure read with no state
    change) drops both — there's nothing that was 'switched out of' when
    nothing was switched, and the banner just states what's active now."""
    envs = manifest.get("environments") or {}
    shared = manifest.get("shared") or {}
    ...
    other_commands_rows = "".join(
        ...
        for cmd, desc in (
            ('status "$CLAUDE_CODE_SESSION_ID"',
             ...
             "none)."),
            ('status "$CLAUDE_CODE_SESSION_ID" --preview [PATH]',
             "Also write an HTML status page for the current state — a "
             "pure read, no switch, nothing changes. Same page shape as "
             "switch --preview, minus the from/to banner and 'Switched "
             "out of' section, since nothing was switched."),
            ('environments',
    ...
    if mode == "status":
        page_title = "Agentic-Gate — environment status"
        banner_html = (f"{_env_dot(new_env_name)}\n"
                       f"    <span class=\"muted\">· checked "
                       f"{escape(time.strftime('%Y-%m-%d %H:%M:%S'))}</span>")
        switched_out_section = ""
    else:
        page_title = "Agentic-Gate — environment switch"
        banner_html = (f"{_env_dot(prev_env_name)}\n"
                       f"    <span class=\"arrow\">→</span>\n"
                       f"    {_env_dot(new_env_name)}\n"
                       f"    <span class=\"muted\">· switched "
                       f"{escape(time.strftime('%Y-%m-%d %H:%M:%S'))}</span>")
        switched_out_section = f"""
...
def selftest() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            ...
            # --- status: a pure read, and its own --preview ---
            state_before = dict(load_state("t3"))
            result = status_cmd(["t3"], quiet=True)
            check("status returns a dict with the expected fields",
                  isinstance(result, dict)
                  and result.get("version") == VERSION
                  and result.get("state", {}).get("active") == "vendor-x")
            check("status is a pure read — no state change",
                  dict(load_state("t3")) == state_before)

            status_preview_default = conf_dir() / "switch-preview.html"
            if status_preview_default.exists():
                status_preview_default.unlink()
            result = status_cmd(["t3", "--preview"], quiet=True)
            check("status --preview succeeds and still returns the status "
                  "dict", isinstance(result, dict))
            check("status --preview writes to the default path when none "
                  "given", status_preview_default.exists())
            status_preview_html = status_preview_default.read_text(encoding="utf-8")
            check("status --preview HTML is titled 'environment status', "
                  "not 'environment switch' — nothing was switched",
                  "environment status" in status_preview_html
                  and "environment switch" not in status_preview_html)
            check("status --preview HTML shows the current active "
                  "environment", "vendor-x" in status_preview_html)
            check("status --preview HTML has no 'Switched out of' section "
                  "— nothing was switched",
                  "<h2>Switched out of</h2>" not in status_preview_html)
            check("status --preview HTML still includes the command "
                  "reference tables",
                  "Switch — your options" in status_preview_html
                  and "Other commands" in status_preview_html)
            check("status --preview HTML has no forbidden top-level tags",
                  not any(tag in status_preview_html.lower() for tag in
                          ("<!doctype", "<html", "<head", "<body")))

            custom_status_preview = Path(td) / "custom-status-preview.html"
            result = status_cmd(["t3", "--preview", str(custom_status_preview)],
                                quiet=True)
            check("status --preview PATH succeeds", isinstance(result, dict))
            check("status --preview PATH writes to the given path",
                  custom_status_preview.exists())
...
def status_cmd(argv, quiet=False):
    """Print (or return, when quiet) this session's active environment,
    the manifest path, and how the guardrail is armed. --preview
    additionally writes an HTML status page for the CURRENT state — a
    pure read, no switch, nothing changes, unlike `switch --preview`
    (same page shape, minus the from/to banner and 'Switched out of'
    section, since nothing was switched)."""
    ...
    while i < len(argv):
        ...
        positional.append(tok)
        i += 1
    sid = positional[0] if positional else "default"
    ...
    result = {"version": VERSION, "manifest": str(manifest_path()),
              "manifest_found": manifest is not None, "armed_via": armed_via,
              "hooks_registered_in_settings": standalone, "state": state}
    if not quiet:
    ...
    if preview_requested:
        if manifest is None:
            print(f"agentic-gate: no manifest at {manifest_path()} — "
                  f"nothing to preview.", file=sys.stderr)
            return 2
        html = _build_switch_preview_html(manifest, state, state.get("active"),
                                          mode="status")
        out_path = (Path(preview_path) if preview_path
                   else conf_dir() / "switch-preview.html")
        out_path.parent.mkdir(parents=True, exist_ok=True)
