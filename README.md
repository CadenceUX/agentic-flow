...
Agentic-Flow is a Claude Code hook that referees calls between
...
Agentic-Flow is a reference implementation of the missing runtime layer:
...
```
/plugin marketplace add CadenceUX/agentic-flow
/plugin install agentic-flow@cadenceux
```

Enable the plugin when prompted — enabling registers the hooks
automatically. Your next session will report:

> *Agentic-Flow v0.2.0 is installed but found no manifest … It is
> DISARMED for this session.*

That's expected: the guardrail never guesses your boundaries. Ask Claude to
**"set up my Agentic-Flow manifest"** — the bundled `agentic-flow-manage`
...
registers the hooks automatically, and the bundled `agentic-flow-manage`
...
```bash
mkdir -p ~/.claude/agentic-flow
cp environments.example.json ~/.claude/agentic-flow/environments.json
...
```bash
python3 agentic-flow.py install
```

`install` copies the engine to `~/.claude/agentic-flow/`, seeds the
...
```bash
python3 ~/.claude/agentic-flow/agentic-flow.py uninstall          # disarm
...
| `~/.claude/agentic-flow/agentic-flow.py` | `install` | `uninstall --purge` |
| `~/.claude/agentic-flow/environments.json` | `install` (seed) + your edits | `uninstall --purge` |
| `~/.claude/agentic-flow/crossings.log` | runtime | `uninstall --purge` |
...
grep -c agentic-flow ~/.claude/settings.json   # expect no matches
```
...
`~/.claude/agentic-flow/environments.json`:
...
- ❌ Cannot stop a plugin's *context injection* (its instructions entering
  the session). That lever is per-project plugin enablement; Agentic-Flow's
