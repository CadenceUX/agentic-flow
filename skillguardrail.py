#!/usr/bin/env python3
"""
SkillGuardrail — no skill crosses environments unannounced.
...
Config: ~/.claude/skillguardrail/environments.json  (see environments.example.json)
Override the config dir with $SKILLGUARDRAIL_HOME (used by selftest).

Usage:
  echo '<hook-json>' | skillguardrail.py     # normal hook invocation
  skillguardrail.py selftest                 # run embedded fixture tests
  skillguardrail.py status [session_id]      # print active state
...
def conf_dir() -> Path:
    return Path(os.environ.get("SKILLGUARDRAIL_HOME",
                               str(Path.home() / ".claude" / "skillguardrail")))
            ...
            if unknown in ("warn", "ask", "deny"):
                ...
                return {"decision": unknown,
                        "reason": f"'{label}' is not declared in any "
                                  f"environment in the SkillGuardrail manifest",
...
def handle_session_start(data: dict) -> dict:
    ...
    if manifest is None:
        ctx = ("SkillGuardrail v" + VERSION + " is installed but found no "
    ...
    policy = manifest.get("policy") or {}
    lines = [
        f"SkillGuardrail v{VERSION} armed. "
...
def handle_pre_tool_use(data: dict) -> dict:
    ...
    if decision == "warn":
        log_crossing({"session": session_id, "tool": tool, "label": label,
                      ...
                      "mode": "warn"})
        return {"systemMessage": f"⚠ SkillGuardrail: {reason}"}
    ...
    return {"hookSpecificOutput": {
        ...
        "permissionDecisionReason": f"SkillGuardrail: {reason}"}}
...
def settings_file() -> Path:
    return Path(os.environ.get("SKILLGUARDRAIL_SETTINGS",
...
def _read_settings():
    ...
    try:
        ...
        raise SystemExit(
            f"skillguardrail: {p} is not valid JSON ({exc}); refusing to "
...
def _write_settings(settings: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    if p.exists():
        shutil.copy2(p, p.with_name(p.name + ".sg-backup"))
    with open(p, "w") as f:
...
def install(argv, quiet=False):
    ...
    say("\nArmed from the next session. Verify:  skillguardrail.py status\n"
        "Selftest:                              skillguardrail.py selftest\n"
        "Undo everything:                       skillguardrail.py uninstall "
        "[--purge]")
...
def uninstall(argv, quiet=False):
    ...
    if changed:
        ...
        say(f"no skillguardrail hooks found in {settings_file()} — nothing "
...
SELFTEST_MANIFEST = {
    "version": 1,
    "environments": {
        "cadenceux": {"skills": ["anthropic-skills:*"],
                      "paths": ["*/skills-plugin/*"]},
        "fmaiac": {"skills": ["FileMaker:*"], "agents": ["FileMaker:*"],
                   "commands": ["fm-build", "patch-runner"],
                   "paths": ["~/.claude/plugins/cache/fmaiac/*"]},
        "agentic-fm": {"mcp": ["mcp__agentic-fm__*"]},
        "proofkit": {"mcp": ["mcp__proofkit__*"]},
    },
    "shared": {"commands": ["fm-clipboard-write", "fm-renderer"]},
    "policy": {"default": "warn", "unknown": "warn",
               "pairs": {"agentic-fm|proofkit": "gate"}},
    "projects": {"/tmp/sgtest-project": "fmaiac"},
}

SELFTEST_CASES = [
    # (description, active_env, tool, tool_input, expected_decision)
    ("same-env skill call", "cadenceux", "Skill",
     {"skill": "anthropic-skills:filemaker-xml"}, "allow"),
    ("cross-env agent dispatch (the FMAIAC incident)", "cadenceux", "Task",
     {"subagent_type": "FileMaker:schema-builder"}, "warn"),
    ("cross-env reference read (the fm-reference incident)", "cadenceux",
     "Read", {"file_path": os.path.expanduser(
         "~/.claude/plugins/cache/fmaiac/FileMaker/0.8.16/tools/"
         "fm-reference/script-steps/SetVariable.md")}, "warn"),
    ("cross-env bash command", "cadenceux", "Bash",
     {"command": "fm-build preview --spec - --saxml saxml/Project/"}, "warn"),
    ("gated MCP pair (agentic-fm → proofkit)", "agentic-fm",
     "mcp__proofkit__validate_cert", {}, "ask"),
    ("shared delivery tool from any env", "cadenceux", "Bash",
     {"command": "fm-clipboard-write XMSC /tmp/x.xml"}, "allow"),
    ("unknown skill follows unknown policy", "fmaiac", "Skill",
     {"skill": "mystery-pack:do-things"}, "warn"),
    ("no active env yet — first skill allowed", None, "Skill",
     {"skill": "FileMaker:calculations"}, "allow"),
    ("unclassified everyday bash is untouched", "cadenceux", "Bash",
     {"command": "ls -la /tmp"}, "allow"),
    ("cross-env skill invocation warns before switching", "fmaiac", "Skill",
     {"skill": "anthropic-skills:fmp-dev-full-script"}, "warn"),
]
...
def selftest() -> int:
    ...
    prev_home = os.environ.get("SKILLGUARDRAIL_HOME")
    prev_settings = os.environ.get("SKILLGUARDRAIL_SETTINGS")
    try:
        with tempfile.TemporaryDirectory() as td:
            conf = Path(td) / "conf"
            conf.mkdir(parents=True)
            os.environ["SKILLGUARDRAIL_HOME"] = str(conf)
            os.environ["SKILLGUARDRAIL_SETTINGS"] = str(
            ...
            check("SessionStart resolves project home env",
                  "Active environment: fmaiac" in ctx)

            save_state("t2", {"active": "fmaiac"})
            handle_post_tool_use(
                {"session_id": "t2", "tool_name": "Skill",
                 "tool_input": {"skill": "anthropic-skills:fmp-dev-full-script"}})
            check("PostToolUse switches active env after Skill runs",
                  load_state("t2").get("active") == "cadenceux")
        ...
        for var, val in (("SKILLGUARDRAIL_HOME", prev_home),
                         ("SKILLGUARDRAIL_SETTINGS", prev_settings)):
            if val is None:
...
def main() -> int:
    ...
    if argv:
        ...
        if verb == "status":
            ...
            return 0
        print(f"skillguardrail: unknown verb '{verb}' "
