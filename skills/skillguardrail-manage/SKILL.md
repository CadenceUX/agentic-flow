---
name: skillguardrail-manage
description: >
  Install, configure, audit, and cleanly uninstall SkillGuardrail — the hook
  that referees cross-environment calls between skill packs, plugins, and MCP
  servers. Use when the user asks to install or set up SkillGuardrail, remove
  or uninstall it, classify a newly installed skill pack or plugin into an
  environment, explain a SkillGuardrail warning or permission prompt, gate or
...
# SkillGuardrail Management

SkillGuardrail is a **hook, not a skill** — its enforcement runs outside the
...
The engine lives at `~/.claude/skillguardrail/skillguardrail.py` once
...
| **Standalone** | `skillguardrail.py install` (writes user `~/.claude/settings.json`) | `skillguardrail.py uninstall` |
...
1. Ask which arming method the user wants (plugin enable vs standalone).
   ...
   ```bash
   python3 /path/to/skillguardrail.py install
   ```

   This (a) copies the engine to `~/.claude/skillguardrail/`, (b) seeds
...
| `~/.claude/skillguardrail/skillguardrail.py` | `install` | `uninstall --purge` only |
| `~/.claude/skillguardrail/environments.json` | `install` (seed) + user edits | `uninstall --purge` only |
| `~/.claude/skillguardrail/crossings.log` | runtime | `uninstall --purge` only |
| `settings.json.sg-backup` | `install`/`uninstall` | manually, when satisfied |
...
```bash
python3 ~/.claude/skillguardrail/skillguardrail.py uninstall          # disarm, keep data
...
```bash
python3 ~/.claude/skillguardrail/skillguardrail.py status   # hooks_registered_in_settings: false
grep -c skillguardrail ~/.claude/settings.json              # expect 0 / no match
```
...
When the user asks "why did SkillGuardrail warn/block?", read the last
entries of `~/.claude/skillguardrail/crossings.log` (JSONL: session, tool,
