---
name: agentic-flow-manage
description: >
  Install, configure, audit, and cleanly uninstall Agentic-Flow — the hook
  that referees cross-environment calls between skill packs, plugins, and MCP
  servers. Use when the user asks to install or set up Agentic-Flow, remove
  or uninstall it, classify a newly installed skill pack or plugin into an
  environment, explain an Agentic-Flow warning or permission prompt, gate or
...
Agentic-Flow is a **hook, not a skill** — its enforcement runs outside the
...
The engine lives at `~/.claude/agentic-flow/agentic-flow.py` once
...
| **Standalone** | `agentic-flow.py install` (writes user `~/.claude/settings.json`) | `agentic-flow.py uninstall` |
...
```bash
python3 ~/.claude/agentic-flow/agentic-flow.py status
```
...
1. Ask which arming method the user wants (plugin enable vs standalone).
   ...
   ```bash
   python3 /path/to/agentic-flow.py install
   ```

   This (a) copies the engine to `~/.claude/agentic-flow/`, (b) seeds
...
| `~/.claude/agentic-flow/state/` | runtime | `uninstall` |
| `~/.claude/agentic-flow/agentic-flow.py` | `install` | `uninstall --purge` only |
| `~/.claude/agentic-flow/environments.json` | `install` (seed) + user edits | `uninstall --purge` only |
| `~/.claude/agentic-flow/crossings.log` | runtime | `uninstall --purge` only |
...
```bash
python3 ~/.claude/agentic-flow/agentic-flow.py uninstall          # disarm, keep data
python3 ~/.claude/agentic-flow/agentic-flow.py uninstall --purge  # remove every trace
```
...
```bash
python3 ~/.claude/agentic-flow/agentic-flow.py status   # hooks_registered_in_settings: false
grep -c agentic-flow ~/.claude/settings.json              # expect 0 / no match
```
...
```bash
python3 ~/.claude/agentic-flow/agentic-flow.py audit
```
...
When the user asks "why did Agentic-Flow warn/block?", read the last
entries of `~/.claude/agentic-flow/crossings.log` (JSONL: session, tool,
