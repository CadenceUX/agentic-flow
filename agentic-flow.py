#!/usr/bin/env python3
"""
Agentic-Flow — no skill crosses environments unannounced.
...
Config: ~/.claude/agentic-flow/environments.json  (see environments.example.json)
Override the config dir with $AGENTIC_FLOW_HOME (used by selftest).

Usage:
  echo '<hook-json>' | agentic-flow.py     # normal hook invocation
  agentic-flow.py selftest                 # run embedded fixture tests
  agentic-flow.py status [session_id]      # print active state
...
def conf_dir() -> Path:
    return Path(os.environ.get("AGENTIC_FLOW_HOME",
                               str(Path.home() / ".claude" / "agentic-flow")))
            ...
            if unknown in ("warn", "ask", "deny"):
                ...
                return {"decision": unknown,
                        "reason": f"'{label}' is not declared in any "
                                  f"environment in the Agentic-Flow manifest",
...
def handle_session_start(data: dict) -> dict:
    ...
    if manifest is None:
        ctx = ("Agentic-Flow v" + VERSION + " is installed but found no "
    ...
    policy = manifest.get("policy") or {}
    lines = [
        f"Agentic-Flow v{VERSION} armed. "
...
def handle_pre_tool_use(data: dict) -> dict:
    ...
    if decision == "warn":
        log_crossing({"session": session_id, "tool": tool, "label": label,
                      ...
                      "mode": "warn"})
        return {"systemMessage": f"⚠ Agentic-Flow: {reason}"}
    ...
    return {"hookSpecificOutput": {
        ...
        "permissionDecisionReason": f"Agentic-Flow: {reason}"}}
...
def settings_file() -> Path:
    return Path(os.environ.get("AGENTIC_FLOW_SETTINGS",
...
def _read_settings():
    ...
    try:
        ...
        raise SystemExit(
            f"agentic-flow: {p} is not valid JSON ({exc}); refusing to "
...
def install(argv, quiet=False):
    ...
    say("\nArmed from the next session. Verify:  agentic-flow.py status\n"
        "Selftest:                              agentic-flow.py selftest\n"
        "Undo everything:                       agentic-flow.py uninstall "
        "[--purge]")
...
def uninstall(argv, quiet=False):
    ...
    if changed:
        ...
        say(f"no agentic-flow hooks found in {settings_file()} — nothing "
...
def selftest() -> int:
    ...
    prev_home = os.environ.get("AGENTIC_FLOW_HOME")
    prev_settings = os.environ.get("AGENTIC_FLOW_SETTINGS")
    try:
        with tempfile.TemporaryDirectory() as td:
            conf = Path(td) / "conf"
            conf.mkdir(parents=True)
            os.environ["AGENTIC_FLOW_HOME"] = str(conf)
            os.environ["AGENTIC_FLOW_SETTINGS"] = str(
        ...
        for var, val in (("AGENTIC_FLOW_HOME", prev_home),
                         ("AGENTIC_FLOW_SETTINGS", prev_settings)):
            if val is None:
...
def main() -> int:
    ...
    if argv:
        ...
        if verb == "status":
            ...
            return 0
        print(f"agentic-flow: unknown verb '{verb}' "
